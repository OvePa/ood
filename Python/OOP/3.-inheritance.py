...
Reusability         We don’t need to duplicate methods inside the child classes 
                    that also occur in the parent classes.

Code modification   Ensures that all changes are localized and inconsistencies are avoided.

Extensibility       We can extend the base class as per the requirements of the 
                    derived class. It provides an easy way to upgrade or enhance 
                    specific parts of a product without changing the core attributes.

Data hiding         A base class can keep some data private so that the derived 
                    class cannot alter it. This concept is called encapsulation.
"""
